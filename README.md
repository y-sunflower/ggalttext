...
You can be more flexible in how the alt text is generated thanks to the `from` argument. text. Since ggplot2 objects can include built-in alt text (via `labs(alt = "Here goes the text")`), you can choose whether to use that text or the one generated by `ggalttext`.
...
You can also choose the language used for generated text with `lang`:

```r
generate_alt_text(plot, lang = "en") # Default value
generate_alt_text(plot, lang = "fr")
generate_alt_text(plot, lang = "de")
```

The `lang` argument applies to text generated by `ggalttext`; existing alt text returned with `from = "auto"` or `from = "origin"` is returned as-is.

<br>

## How to make accessible visualizations?

`ggalttext` will generate high-quality alternative text, provided your chart design follows accessibility guidelines:
