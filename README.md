...
You can be more flexible in how the alt text is generated thanks to the `source` argument. Since ggplot2 objects can include built-in alt text (via `labs(alt = "Here goes the text")`), you can choose whether to use that text or the one generated by `ggalttext`.

```r
generate_alt_text(plot, source = "default") # Default value
generate_alt_text(plot, source = "auto")
generate_alt_text(plot, source = "origin")
```
