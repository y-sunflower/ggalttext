...
You can choose where the alt text comes from with the `source` argument. Since ggplot2 objects can include built-in alt text via `labs(alt = "Here goes the text")`, `source` can use that text or the one generated by `ggalttext`.
